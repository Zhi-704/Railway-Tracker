DROP TABLE IF EXISTS incident, cancellation, station_performance_archive, user, cancel_code, station, subscription, waypoint, operator;
...
CREATE TABLE cancellation(
    ...
    cancel_code BIGINT NOT NULL REFERENCES cancel_code(cancel_code),
...
CREATE TABLE cancel_code(
    cancel_code CHAR(2) GENERATED ALWAYS AS IDENTITY,
    cause TEXT NOT NULL UNIQUE
);

CREATE TABLE station(
    station_crs CHAR(3) GENERATED ALWAYS AS IDENTITY,
    station_name TEXT NOT NULL UNIQUE
);

CREATE TABLE subscription(
    subscription_id SMALLINT GENERATED ALWAYS AS IDENTITY,
    user_id SMALLINT NOT NULL REFERENCES user(user_id),
    operator_code CHAR(2) NOT NULL REFERENCES operator(operator_code)
);

CREATE TABLE waypoint(
    waypoint_id BIGINT GENERATED ALWAYS AS IDENTITY,
...
CREATE TABLE operator(
    operator_code CHAR(2) GENERATED ALWAYS AS IDENTITY,
